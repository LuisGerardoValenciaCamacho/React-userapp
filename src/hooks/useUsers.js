import { useReducer, useState } from "react";
import { useNavigate } from "react-router-dom";
import Swal from "sweetalert2";
import { usersReducer } from "../reducers/usersReducer";
import { get, post, put, remove } from "../utils/useFetch";

const initialUsers = [
    
];

const initialUserForm = {
    id: 0,
    username: '',
    password: '',
    email: '',
}

export const useUsers = () => {
    const [users, dispatch] = useReducer(usersReducer, initialUsers);
    const [userSelected, setUserSelected] = useState(initialUserForm);
    const [visibleForm, setVisibleForm] = useState(false);
    const navigate = useNavigate();

    const handlerAddUser = async (user) => {
        const response = user.id === 0 ? await post("http://localhost:5500/api/user/create", user) : await put(`http://localhost:5500/api/user/update/${user.id}`, user)        
        if(response.status !== 201) {
            Swal.fire("Error", response.response.data.message, "error");
            return;
        }
        dispatch({
            type: user.id === 0 ? "addUser" : "updateUser",
            payload: response.data
        })
        Swal.fire(user.id === 0 ? "Usuario creado" : "Usuario actualizado", user.id === 0 ? "El usuario ha sido creado con exito!" : "El usuario ha sido actualizado con exito!", "success")
        handlerCloseForm();
        navigate("/users")
    }

    const handlerRemoveUser = (id) => {
        Swal.fire({
            title: 'Esta seguro que desea eliminar?',
            text: "Cuidado el usuario sera eliminado!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Si, eliminar!'
        }).then((result) => {
            if (result.isConfirmed) {
                remove(`http://localhost:5500/api/user/delete/${id}`)
                dispatch({
                    type: 'removeUser',
                    payload: id,
                });
                Swal.fire('Usuario Eliminado!', 'El usuario ha sido eliminado con exito!', 'success')
            }
        })

    }

    const getUsers = async () => {
        await get("http://localhost:5500/api/user/all").then(response => {
            dispatch({
                type: "loadingUsers",
                payload: response.data
            })
        }).catch(error => {
            console.error(error.response.data.message);
            Swal.fire("Error", error.response.data.message, "error");
        })
    }

    const handlerUserSelectedForm = (user) => {
        setVisibleForm(true);
        setUserSelected({ ...user });
    }

    const handlerOpenForm = () => {
        setVisibleForm(true);
    }

    const handlerCloseForm = () => {
        setVisibleForm(false);
        setUserSelected(initialUserForm);
    }
    return {
        users,
        userSelected,
        initialUserForm,
        visibleForm,
        handlerAddUser,
        handlerRemoveUser,
        handlerUserSelectedForm,
        handlerOpenForm,
        handlerCloseForm,
        getUsers
    }
}